...
Image *RGB2CMY(Image *rgb_image_ptr) {
    int rows = rgb_image_ptr->rows;
    int cols = rgb_image_ptr->cols;
    colors color = rgb_image_ptr->color;

    Image *cmy_image_ptr = create_image(rows, cols, color);
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++) {
            for (int k = 0; k < color; k++) {
                (cmy_image_ptr->image)[i][j][k] = MAX_INTENSITY - (rgb_image_ptr->image)[i][j][k];
            }
        }
    }

    return cmy_image_ptr;
}

Image *CMY2RGB(Image *cmy_image_ptr) {
    int rows = cmy_image_ptr->rows;
    int cols = cmy_image_ptr->cols;
    colors color = cmy_image_ptr->color;

    Image *rgb_image_ptr = create_image(rows, cols, color);
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++) {
            for (int k = 0; k < color; k++) {
                (rgb_image_ptr->image)[i][j][k] = MAX_INTENSITY - (cmy_image_ptr->image)[i][j][k];
            }
        }
    }
